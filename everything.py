...
try:
    print("Running...")
    #this value is 14101 for left USB port and 14201 for right, in my experience (mac)
    #COM3 for windows
    ...
    nextsteppertime = time.time()
    steppervpos = 200 #999 is the bottom
    rot = 500
    shoot = 0
    ...
    def overlay_to_rotated_region(background_img, overlay_img, dst_pts, alpha=1, blur=3, erode=3, transparency = True):
        ...
        amax = np.amax(local_dst_pts, 0);
        overlay = cv2.warpPerspective(overlay_img, overlay_M, (int(amax[0]),int(amax[1])))
    ...
    while rval and cv2.getWindowProperty("frame", cv2.WND_PROP_VISIBLE) >= 1:
            ...
            for i in range(k):
                ...
                if prevclusters is not None:
                    ...
                    if idealcluster != -1:
                        ...
                        cv2.line(frame, (int(means[i][1]),int(means[i][0])), (int(prevmeans[idealcluster][1]),int(prevmeans[idealcluster][0])), (0, 0, 255), 4)
        ...
        if is_tracking:
            ...
            if mobile == True and predictedcentroid is not None:
                ...
                '''
                #print(predictedcentroid)
                steppervpos = int(999 * (1 - centroid[1]/frame.shape[0]))
                #servovpos = (frame.shape[0] / 2 - centroid[1]) * movecoefficient
                if steppervpos < 0:
                    steppervpos = 0
                elif steppervpos > 999:
                    steppervpos = 999

                rot = int(999 * centroid[0]/frame.shape[1])
                if rot < 0:
                    rot = 0
                elif rot > 999:
                    rot = 999
        ...
        if time.time() > nextsteppertime:
            if armed and primecountdown > 0:
                shoot = 1
            else:
                shoot = 0
            ser.write(bytearray([int(steppervpos/100), int((steppervpos%100)/10), (steppervpos%100)%10, int(rot/100), int((rot%100)/10), (rot%100)%10, shoot]))
            nextsteppertime = time.time() + 0.02
            print("wriiiite", steppervpos, " and ", rot, " and ", shoot)
    ...
    None
ser.write(bytearray([int(steppervpos/100), int((steppervpos%100)/10), (steppervpos%100)%10, int(rot/100), int((rot%100)/10), (rot%100)%10, 0]))
